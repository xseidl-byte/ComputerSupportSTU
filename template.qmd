...
```{r}
# code for solving task
    ggplot(my_data, aes(x = lat, y = population)) +
    geom_point(alpha = 0.5, size = 2) +
    geom_smooth() +
    labs(
    title = "Population vs Latitude",
    x = "Latitude",
    y = "Population"
    ) +
    theme_minimal()
...
# 2. načítanie dát
  my_data <- read.csv("worldcities.csv")

# 3. pridanie kontinentu podľa krajiny
  continent_map <- list(
    Africa = c("Nigeria","Egypt","South Africa","Kenya","Morocco","Algeria",
               "Tanzania","Ghana","Ethiopia","Uganda","Angola","Senegal",
               "Ivory Coast","Mozambique","Cameroon"),
    Asia = c("China","India","Japan","South Korea","Indonesia","Saudi Arabia",
             "Iran","Iraq","Vietnam","Thailand","Philippines","Malaysia",
             "Pakistan","Bangladesh","Nepal","Israel","Turkey"),
    Europe = c("Slovakia","Czech Republic","Germany","France","UK","Italy",
               "Spain","Portugal","Austria","Hungary","Poland","Ukraine",
               "Romania","Sweden","Norway","Finland","Belgium","Netherlands",
               "Greece","Switzerland"),
    North_America = c("United States","Canada","Mexico","Cuba","Dominican Republic"),
    South_America = c("Brazil","Argentina","Chile","Peru","Colombia","Ecuador","Uruguay"),
    Oceania = c("Australia","New Zealand","Fiji","Papua New Guinea")
  )
  
  my_data$continent <- NA
  
  for (cont in names(continent_map)) {
    my_data$continent[my_data$country %in% continent_map[[cont]]] <- cont
  }

# 4. vynechanie riadkov bez kontinentu
  my_data <- my_data %>% filter(!is.na(continent))

# 5.3 BOX PLOT – populácia podľa kontinentu
    ggplot(my_data, aes(x = continent, y = population, fill = continent)) +
      geom_boxplot() +
      labs(
...
# 5.4 — Facetted Histogram by Continent
```{r}
# code for solving task
    ggplot(my_data, aes(x = population)) +
